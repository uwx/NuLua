...
using static NuLua.Interop.Lua52.NativeMethods;

namespace NuLua.Interop.Lua52;

public sealed unsafe class Lua52Operations : ILuaOperations
{
    public static readonly Lua52Operations Default = new();
    ...
    public void lua_rotate(void* state, int index, int count) => throw new NotSupportedException("lua_rotate is not supported in Lua 5.2.");
    ...
    public nuint lua_objlen(void* state, int index) => NativeMethods.lua_rawlen((lua_State*)state, index);
    public nuint lua_rawlen(void* state, int index) => NativeMethods.lua_rawlen((lua_State*)state, index);
    ...
    public nuint lua_stringtonumber(void* state, byte* value) => throw new NotSupportedException("lua_stringtonumber is not supported in Lua 5.2.");
    ...
    {
        NativeMethods.lua_getglobal((lua_State*)state, name);
    ...
    public int lua_geti(void* state, int index, long key) => throw new NotSupportedException("lua_geti is not supported in Lua 5.2.");
    ...
    public int lua_rawgetp(void* state, int index, void* pointer)
    {
        NativeMethods.lua_rawgetp((lua_State*)state, index, pointer);
        return NativeMethods.lua_type((lua_State*)state, -1);
    }
    ...
    {
        if (userValueCount != 1) throw new NotSupportedException("lua_newuserdatauv with userValueCount != 1 is not supported in Lua 5.2.");
    ...
    public void lua_getfenv(void* state, int index) => throw new NotSupportedException("lua_getfenv is not supported in Lua 5.2.");

    public int lua_getuservalue(void* state, int index)
    {
        NativeMethods.lua_getuservalue((lua_State*)state, index);
        return NativeMethods.lua_type((lua_State*)state, -1);
    }

    public int lua_getiuservalue(void* state, int index, int userValueIndex)
    {
        if (userValueIndex != 1) throw new NotSupportedException("lua_getiuservalue with userValueIndex != 1 is not supported in Lua 5.2.");
        NativeMethods.lua_getuservalue((lua_State*)state, index);
        return NativeMethods.lua_type((lua_State*)state, -1);
    }
    ...
    public void lua_seti(void* state, int index, long key) => throw new NotSupportedException("lua_seti is not supported in Lua 5.2.");
    ...
    public int lua_setfenv(void* state, int index) => throw new NotSupportedException("lua_setfenv is not supported in Lua 5.2.");
    public void lua_setuservalue(void* state, int index) => NativeMethods.lua_setuservalue((lua_State*)state, index);

    public int lua_setiuservalue(void* state, int index, int userValueIndex)
    {
        if (userValueIndex != 1) throw new NotSupportedException("lua_setiuservalue with userValueIndex != 1 is not supported in Lua 5.2.");
        NativeMethods.lua_setuservalue((lua_State*)state, index);
        return 1;
    }
    ...
    {
        if (continuation != null) throw new NotSupportedException("lua_callk continuation signature differs in Lua 5.2; pass null continuation.");
        NativeMethods.lua_callk((lua_State*)state, argumentCount, resultCount, (int)context, null);
    }
    ...
    {
        if (continuation != null) throw new NotSupportedException("lua_pcallk continuation signature differs in Lua 5.2; pass null continuation.");
        return NativeMethods.lua_pcallk((lua_State*)state, argumentCount, resultCount, errorFunction, (int)context, null);
    }

    public int lua_cpcall(void* state, delegate* unmanaged[Cdecl]<void*, int> function, void* userData) => throw new NotSupportedException("lua_cpcall is not supported in Lua 5.2.");
    ...
    {
        if (continuation != null) throw new NotSupportedException("lua_yieldk continuation signature differs in Lua 5.2; pass null continuation.");
        return NativeMethods.lua_yieldk((lua_State*)state, resultCount, (int)context, null);
    }
    ...
    public int lua_isyieldable(void* state) => throw new NotSupportedException("lua_isyieldable is not supported in Lua 5.2.");
    ...
    public int luaopen_utf8(void* state) => throw new NotSupportedException("luaopen_utf8 is not supported in Lua 5.2.");
    ...
    public void luaL_openlib(void* state, byte* libraryName, void* registrations, int upvalueCount) => throw new NotSupportedException("luaL_openlib is not supported in Lua 5.2.");
    public void luaL_register(void* state, byte* libraryName, void* registrations) => throw new NotSupportedException("luaL_register is not supported in Lua 5.2.");
    public void luaL_checkversion_(void* state, double version, nuint size) => NativeMethods.luaL_checkversion_((lua_State*)state, version);
    ...
    public int luaL_typerror(void* state, int argument, byte* typeName) => throw new NotSupportedException("luaL_typerror is not supported in Lua 5.2.");
    ...
    public byte* luaL_findtable(void* state, int index, byte* fieldName, int sizeHint) => throw new NotSupportedException("luaL_findtable is not supported in Lua 5.2.");
