...
using static NuLua.Interop.Lua51.NativeMethods;

namespace NuLua.Interop.Lua51;

public sealed unsafe class Lua51Operations : ILuaOperations
{
    public static readonly Lua51Operations Default = new();
    ...
    public int lua_absindex(void* state, int index)
    {
        if (index > 0 || index <= -10000) return index;
        return NativeMethods.lua_gettop((lua_State*)state) + 1 + index;
    }
    ...
    public void lua_rotate(void* state, int index, int count) => throw new NotSupportedException("lua_rotate is not supported in Lua 5.1.");
    public void lua_copy(void* state, int fromIndex, int toIndex) => throw new NotSupportedException("lua_copy is not supported in Lua 5.1.");
    ...
    public int lua_isinteger(void* state, int index) => 0;
    ...
    public int lua_compare(void* state, int index1, int index2, int operation) => throw new NotSupportedException("lua_compare is not supported in Lua 5.1.");

    public double lua_tonumber(void* state, int index) => NativeMethods.lua_tonumber((lua_State*)state, index);

    public double lua_tonumberx(void* state, int index, int* isNumber)
    {
        var L = (lua_State*)state;
        var value = NativeMethods.lua_tonumber(L, index);
        if (isNumber != null) *isNumber = value != 0 || NativeMethods.lua_isnumber(L, index) != 0 ? 1 : 0;
        return value;
    }

    public long lua_tointeger(void* state, int index) => NativeMethods.lua_tointeger((lua_State*)state, index);

    public long lua_tointegerx(void* state, int index, int* isNumber)
    {
        var L = (lua_State*)state;
        var value = NativeMethods.lua_tointeger(L, index);
        if (isNumber != null) *isNumber = value != 0 || NativeMethods.lua_isnumber(L, index) != 0 ? 1 : 0;
        return value;
    }
    ...
    public nuint lua_objlen(void* state, int index) => NativeMethods.lua_objlen((lua_State*)state, index);
    public nuint lua_rawlen(void* state, int index) => NativeMethods.lua_objlen((lua_State*)state, index);
    ...
    public void lua_arith(void* state, int operation) => throw new NotSupportedException("lua_arith is not supported in Lua 5.1.");
    public void lua_len(void* state, int index) => throw new NotSupportedException("lua_len is not supported in Lua 5.1.");
    public nuint lua_stringtonumber(void* state, byte* value) => throw new NotSupportedException("lua_stringtonumber is not supported in Lua 5.1.");
    ...
    public void lua_pushinteger(void* state, long value) => NativeMethods.lua_pushinteger((lua_State*)state, (nint)value);
    ...
    public int lua_getglobal(void* state, byte* name)
    {
        var L = (lua_State*)state;
        NativeMethods.lua_getfield(L, LUA_GLOBALSINDEX, name);
        return NativeMethods.lua_type(L, -1);
    }

    public int lua_gettable(void* state, int index)
    {
        NativeMethods.lua_gettable((lua_State*)state, index);
        return NativeMethods.lua_type((lua_State*)state, -1);
    }

    public int lua_getfield(void* state, int index, byte* key)
    {
        NativeMethods.lua_getfield((lua_State*)state, index, key);
        return NativeMethods.lua_type((lua_State*)state, -1);
    }

    public int lua_geti(void* state, int index, long key) => throw new NotSupportedException("lua_geti is not supported in Lua 5.1.");

    public int lua_rawget(void* state, int index)
    {
        NativeMethods.lua_rawget((lua_State*)state, index);
        return NativeMethods.lua_type((lua_State*)state, -1);
    }

    public int lua_rawgeti(void* state, int index, long key)
    {
        NativeMethods.lua_rawgeti((lua_State*)state, index, (int)key);
        return NativeMethods.lua_type((lua_State*)state, -1);
    }

    public int lua_rawgetp(void* state, int index, void* pointer) => throw new NotSupportedException("lua_rawgetp is not supported in Lua 5.1.");
    ...
    public void* lua_newuserdata(void* state, nuint size) => NativeMethods.lua_newuserdata((lua_State*)state, size);

    public void* lua_newuserdatauv(void* state, nuint size, int userValueCount)
    {
        if (userValueCount != 1) throw new NotSupportedException("lua_newuserdatauv with userValueCount != 1 is not supported in Lua 5.1.");
        return NativeMethods.lua_newuserdata((lua_State*)state, size);
    }
    ...
    public int lua_getuservalue(void* state, int index)
    {
        NativeMethods.lua_getfenv((lua_State*)state, index);
        return NativeMethods.lua_type((lua_State*)state, -1);
    }

    public int lua_getiuservalue(void* state, int index, int userValueIndex)
    {
        if (userValueIndex != 1) throw new NotSupportedException("lua_getiuservalue with userValueIndex != 1 is not supported in Lua 5.1.");
        NativeMethods.lua_getfenv((lua_State*)state, index);
        return NativeMethods.lua_type((lua_State*)state, -1);
    }
    ...
    public void lua_seti(void* state, int index, long key) => throw new NotSupportedException("lua_seti is not supported in Lua 5.1.");
    ...
    public void lua_rawsetp(void* state, int index, void* pointer) => throw new NotSupportedException("lua_rawsetp is not supported in Lua 5.1.");
    ...
    public void lua_setuservalue(void* state, int index) => NativeMethods.lua_setfenv((lua_State*)state, index);

    public int lua_setiuservalue(void* state, int index, int userValueIndex)
    {
        if (userValueIndex != 1) throw new NotSupportedException("lua_setiuservalue with userValueIndex != 1 is not supported in Lua 5.1.");
        return NativeMethods.lua_setfenv((lua_State*)state, index);
    }
    ...
    {
        if (continuation != null) throw new NotSupportedException("lua_callk is not supported in Lua 5.1.");
    ...
    {
        if (continuation != null) throw new NotSupportedException("lua_pcallk is not supported in Lua 5.1.");
    ...
    public int lua_load(void* state, delegate* unmanaged[Cdecl]<void*, void*, nuint*, byte*> reader, void* data, byte* chunkName, byte* mode)
        => NativeMethods.lua_load((lua_State*)state, (delegate* unmanaged[Cdecl]<lua_State*, void*, nuint*, byte*>)reader, data, chunkName);

    public int lua_dump(void* state, delegate* unmanaged[Cdecl]<void*, void*, nuint, void*, int> writer, void* data, int strip)
        => NativeMethods.lua_dump((lua_State*)state, (delegate* unmanaged[Cdecl]<lua_State*, void*, nuint, void*, int>)writer, data);
    ...
    {
        if (continuation != null) throw new NotSupportedException("lua_yieldk is not supported in Lua 5.1.");
        return NativeMethods.lua_yield((lua_State*)state, resultCount);
    }

    public int lua_resume(void* state, void* fromState, int argumentCount, int* resultCount) => NativeMethods.lua_resume((lua_State*)state, argumentCount);
    public int lua_status(void* state) => NativeMethods.lua_status((lua_State*)state);
    public int lua_isyieldable(void* state) => throw new NotSupportedException("lua_isyieldable is not supported in Lua 5.1.");
    ...
    public void* lua_upvalueid(void* state, int functionIndex, int index) => throw new NotSupportedException("lua_upvalueid is not supported in Lua 5.1.");
    public void lua_upvaluejoin(void* state, int functionIndex1, int index1, int functionIndex2, int index2) => throw new NotSupportedException("lua_upvaluejoin is not supported in Lua 5.1.");
    ...
    public int luaopen_coroutine(void* state) => throw new NotSupportedException("luaopen_coroutine is not supported in Lua 5.1.");
    ...
    public int luaopen_utf8(void* state) => throw new NotSupportedException("luaopen_utf8 is not supported in Lua 5.1.");
    public int luaopen_bit32(void* state) => throw new NotSupportedException("luaopen_bit32 is not supported in Lua 5.1.");
    ...
    public void luaL_openlib(void* state, byte* libraryName, void* registrations, int upvalueCount) => NativeMethods.luaL_openlib((lua_State*)state, libraryName, (luaL_Reg*)registrations, upvalueCount);
    ...
    public void luaL_checkversion_(void* state, double version, nuint size) => throw new NotSupportedException("luaL_checkversion_ is not supported in Lua 5.1.");
    ...
    public byte* luaL_tolstring(void* state, int index, nuint* length) => throw new NotSupportedException("luaL_tolstring is not supported in Lua 5.1.");
    public int luaL_typerror(void* state, int argument, byte* typeName) => NativeMethods.luaL_typerror((lua_State*)state, argument, typeName);
    ...
    public long luaL_optinteger(void* state, int argument, long defaultValue) => NativeMethods.luaL_optinteger((lua_State*)state, argument, (nint)defaultValue);
    ...
    public void luaL_setmetatable(void* state, byte* typeName) => throw new NotSupportedException("luaL_setmetatable is not supported in Lua 5.1.");
    public void* luaL_testudata(void* state, int userDataIndex, byte* typeName) => throw new NotSupportedException("luaL_testudata is not supported in Lua 5.1.");
    ...
    public int luaL_fileresult(void* state, int status, byte* fileName) => throw new NotSupportedException("luaL_fileresult is not supported in Lua 5.1.");
    public int luaL_execresult(void* state, int status) => throw new NotSupportedException("luaL_execresult is not supported in Lua 5.1.");
    ...
    public int luaL_loadfile(void* state, byte* fileName) => NativeMethods.luaL_loadfile((lua_State*)state, fileName);
    public int luaL_loadfilex(void* state, byte* fileName, byte* mode) => NativeMethods.luaL_loadfile((lua_State*)state, fileName);
    public int luaL_loadbuffer(void* state, byte* buffer, nuint size, byte* name) => NativeMethods.luaL_loadbuffer((lua_State*)state, buffer, size, name);
    public int luaL_loadbufferx(void* state, byte* buffer, nuint size, byte* name, byte* mode) => NativeMethods.luaL_loadbuffer((lua_State*)state, buffer, size, name);
    public int luaL_loadstring(void* state, byte* source) => NativeMethods.luaL_loadstring((lua_State*)state, source);
    public long luaL_len(void* state, int index) => throw new NotSupportedException("luaL_len is not supported in Lua 5.1.");
    ...
    public void luaL_setfuncs(void* state, void* registrations, int upvalueCount) => throw new NotSupportedException("luaL_setfuncs is not supported in Lua 5.1.");
    public int luaL_getsubtable(void* state, int index, byte* fieldName) => throw new NotSupportedException("luaL_getsubtable is not supported in Lua 5.1.");
    public void luaL_traceback(void* state, void* sourceState, byte* message, int level) => throw new NotSupportedException("luaL_traceback is not supported in Lua 5.1.");
    public void luaL_requiref(void* state, byte* moduleName, delegate* unmanaged[Cdecl]<void*, int> openFunction, int global) => throw new NotSupportedException("luaL_requiref is not supported in Lua 5.1.");
    public void luaL_buffinit(void* state, void* buffer) => NativeMethods.luaL_buffinit((lua_State*)state, (luaL_Buffer*)buffer);
    public byte* luaL_prepbuffer(void* buffer) => NativeMethods.luaL_prepbuffer((luaL_Buffer*)buffer);
    public byte* luaL_prepbuffsize(void* buffer, nuint size) => NativeMethods.luaL_prepbuffer((luaL_Buffer*)buffer);
    public byte* luaL_buffinitsize(void* state, void* buffer, nuint size) => throw new NotSupportedException("luaL_buffinitsize is not supported in Lua 5.1.");
    ...
    public void luaL_pushresultsize(void* buffer, nuint size) => throw new NotSupportedException("luaL_pushresultsize is not supported in Lua 5.1.");
}
