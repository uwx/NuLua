...
{
    LuaHook<Lua55State>? hookDelegate;
    ...
    void ILuaDebug<Lua55State>.SetHook(LuaHook<Lua55State>? hook, LuaHookMask mask, int count)
    {
        CheckDisposed();
    ...
    LuaHook<Lua55State>? ILuaDebug<Lua55State>.GetHook() => hookDelegate;

    LuaHook? ILuaDebug.GetHook()
    {
        if (nonGenericHookDelegate != null) return nonGenericHookDelegate;
        var typed = hookDelegate;
        return typed == null ? null : (s, ev, line) => typed((Lua55State)s, ev, line);
    }
