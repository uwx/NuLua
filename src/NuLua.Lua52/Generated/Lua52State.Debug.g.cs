...
{
    LuaHook<Lua52State>? hookDelegate;
    ...
    void ILuaDebug<Lua52State>.SetHook(LuaHook<Lua52State>? hook, LuaHookMask mask, int count)
    {
        CheckDisposed();
    ...
    LuaHook<Lua52State>? ILuaDebug<Lua52State>.GetHook() => hookDelegate;

    LuaHook? ILuaDebug.GetHook()
    {
        if (nonGenericHookDelegate != null) return nonGenericHookDelegate;
        var typed = hookDelegate;
        return typed == null ? null : (s, ev, line) => typed((Lua52State)s, ev, line);
    }
